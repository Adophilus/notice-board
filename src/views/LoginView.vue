...
export default {
  ...
  methods: {
    async loginUser() {
      let admin = await Admin.get(
        this.$root.db,
        { where: { email: this.email } },
        false
      )
      admin = admin[0]
      let student = await Student.get(
        this.$root.db,
        { where: { email: this.email } },
        false
      )
      ...
      if (
        admin != null &&
        Admin.is(admin.fields._id) &&
        (await admin.hasPassword(this.password))
      ) {
        this.$store.commit('setUser', admin.fields)
        this.$router.push({ name: 'AdminDashboardView' })
      } else if (
        student != null &&
        Student.is(student.fields._id) &&
        (await student.hasPassword(this.password))
      ) {
