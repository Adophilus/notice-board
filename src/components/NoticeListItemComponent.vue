<template>
  <tr>
    ...
    <td class="align-middle text-center text-sm">
      <span v-if="isPosted" class="badge badge-sm bg-gradient-primary">{{
        status
      }}</span>
    ...
    <td class="align-middle text-center">
      <span class="text-secondary text-xs font-weight-bold">{{
        posted ? moment(posted).format('DD/MM/YYYY') : 'NO'
      }}</span>
    </td>
    <td class="align-middle text-center">
      <i
        class="material-icons"
        v-show="$store.getters.isAdmin && !posted"
        @click="$emit('post-notice', { _id })"
        role="button"
        >post_add</i
      >
      <i
